...
#**********指定参数：
target_id = 89    #要爬取google scholar 主页上的第几篇(从0开始）
...
#-----------------只爬取第 target_id+1 篇论文的引用-----------------------------
        if  article_id != target_id :
            continue
        base_url = info["cite_url"]                    # ”引用“ 界面
        name = info["name"]                            #  该文章名字
        get_all_cite_name_list(d, base_url, article_id, new_articles)
        print('爬取完成')

#get_all_cite_name_list 只爬取所有引用文章的名字，保存到data/articles.json文件中
def get_all_cite_name_list(d, base_url, article_id, new_articles):
    all_cite_name_list = []    #每篇文章的被引用列表
    for start in range(0, 100000, 10):
        print("Page {}".format(start // 10))
        url = get_specify_url(base_url, start)    #每一页引用文章列表的网址，数字按10递增，因为google scholar每页显示10篇引用文章
        enter_url(d, url)                         #打开该页
        cite_name_list = get_cite_name_list(d)    #获取该页的引用文章列表
        if len(cite_name_list) == 0:              #该页为空则退出循环
            break
