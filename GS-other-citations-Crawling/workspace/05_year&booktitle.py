...
#4月26日改进：
#   为了实现在google_scholar上爬取年份和发表位置，在02_citation.py基础上进行了拓展，开发完成后用本文件替换02_文件，并命名为02_citation.py后，一切如旧版本运行
#4月27日改进：
#   通过登录Google账户，运用账户内预先设置好的开启cookie和Google_Scholar内打开了open in BibTeX按钮，实现电机open in BibTeX爬取所有信息
#   但是有个小BUG：最后几篇可能爬取不到
#   爬取结果保存在：/data/cite_info.json
#4月28日改进：
#   解决了最后几篇爬取不到的问题
#   本程序基本开发完成，有个缺点：必须盯着爬取界面，防止Google弹出人机验证时来不及点，导致爬取失败
